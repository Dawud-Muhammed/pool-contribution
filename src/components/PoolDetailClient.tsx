...
type LedgerData = { pool: { id: string; name: string; purpose: string; beneficiary: string; paymentProvider: string | null; paymentAccount: string | null; paymentInstructions: string; capAmount: string; currency: string; status: string; totalAllocated: string; progressPercentage: number }; entries: { id: string; pseudonym: string; type: string; amount: string; createdAt: string }[] };
...
export default function PoolDetailClient({ poolId }: { poolId: string }) {
  ...
  return <main className="shell page-space"><div className="detail-heading"><div><Link className="back-link" href="/pools">← All pools</Link><span className="eyebrow">Public pool dashboard</span><h1>{data.pool.name}</h1><p className="lede">{data.pool.purpose || "A shared community goal tracked with verified contributions and a public ledger."}</p></div><span className={`status-badge ${data.pool.status}`}>{data.pool.status}</span></div>
    <section className="pool-context"><div><span className="eyebrow">Beneficiary</span><strong>{data.pool.beneficiary || "Pool organizer"}</strong></div><div><span className="eyebrow">How to contribute</span><p>{data.pool.paymentInstructions || "Send your payment to the collection account provided by the organizer, then submit the receipt for verification."}</p>{data.pool.paymentProvider && <small>{data.pool.paymentProvider}{data.pool.paymentAccount ? ` · ${data.pool.paymentAccount}` : ""}</small>}</div></section>
